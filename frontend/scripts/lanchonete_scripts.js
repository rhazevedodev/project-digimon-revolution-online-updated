document.addEventListener('DOMContentLoaded', function () {
    const apiURL = 'http://localhost:8080/api/telaMissoes/carregaTelaMissoes';
    ...
    const apiUrlLanchonete = 'http://localhost:8080/api/telaLanchonete/fazerLanche'
    ...
    function carregarCaixaMissao(dataMissoes) {
        ...
        if (resgatarButton) {
            resgatarButton.addEventListener('click', function () {
                ...
                resgatarRecompensaMissao(localStorage.getItem('idDigimon'));
            });
        }
    ...
    document.addEventListener('click', function (event) {
        if (event.target && event.target.id === 'button-comer') {
            // Abrir o modal
            const modal = document.getElementById('resultadoModal');
            modal.style.display = 'block';
            fazerLanche();
    ...
    function fazerLanche() {
        ...
        fetch(apiUrlLanchonete, requestOptions)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Erro na rede, status: ' + response.status);
                }
